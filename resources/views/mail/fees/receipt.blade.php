...
- Bill: {{ $fee->fee_type }}
- Amount paid: {{ ($business ?? $fee->business)?->displayCurrency() ?? 'UGX' }} {{ number_format((float) $fee->amount_paid, 0) }}
- Balance due: {{ ($business ?? $fee->business)?->displayCurrency() ?? 'UGX' }} {{ number_format((float) $fee->remainingBalance(), 0) }}
