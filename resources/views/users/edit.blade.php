    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                ...
                @php
                    $fallbackAvatar = 'https://ui-avatars.com/api/?name=' . urlencode($user->name ?? 'User') . '&background=random';
                    $hasExistingPhoto = !empty($user->profile_photo_path);
                    $previewSrc = $user->profile_photo_url ?: $fallbackAvatar;
                @endphp
                            ...
                            <div class="flex items-center space-x-6">
                                <div class="flex-shrink-0">
                                    <img id="photoPreview"
                                         ...
                                         style="{{ $hasExistingPhoto ? 'display: block;' : 'display: none;' }}">

                                    <div id="photoPlaceholder"
                                         class="h-24 w-24 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center"
                                         style="{{ $hasExistingPhoto ? 'display: none;' : 'display: flex;' }}">
                                    ...
                                    <div>
                                        ...
                                        <select name="business_id"
                                            ...
                                            @foreach($businesses as $id => $name)
                                                <option value="{{ $id }}" {{ (string) old('business_id', $user->business_id) === (string) $id ? 'selected' : '' }}>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                        @error('business_id')
                                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                ...
                                <div>
                                    ...
                                    <select name="branch_id"
                                        ...
                                        @foreach($branches as $id => $name)
                                            <option value="{{ $id }}" {{ (string) old('branch_id', $user->branch_id) === (string) $id ? 'selected' : '' }}>{{ $name }}</option>
                                        @endforeach
                                    </select>
                                    @error('branch_id')
                                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                    ...
                                    <div>
                                        ...
                                        <select name="class_room_id"
                                            ...
                                            @foreach($classRooms as $id => $name)
                                                <option value="{{ $id }}" {{ (string) old('class_room_id', $user->class_room_id) === (string) $id ? 'selected' : '' }}>{{ $name }}</option>
                                            @endforeach
                                        </select>
