        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8">
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    {{ $isChurch ? __('Edit Child') : __('Edit Student') }}
                </h2>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
                ...
                @php
                    ...
                    $previewSrc = $hasPhoto ? asset('storage/' . $student->photo) : 'https://ui-avatars.com/api/?name=Student&background=random';
                @endphp

                <form action="{{ route('school-management.students.update', $student) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="flex items-center space-x-6">
                                <div class="flex-shrink-0">
                                    ...
                                    <div id="photoPlaceholder"
                                         class="h-24 w-24 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center"
                                ...
                                <div>
                                    ...
                                    <select name="parent_guardian_id"
                                        ...
                                        @foreach($parentGuardians as $id => $name)
                                            <option value="{{ $id }}" {{ (string) old('parent_guardian_id', $student->parent_guardian_id) === (string) $id ? 'selected' : '' }}>{{ $name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="class_room_id"
                                        ...
                                        @foreach($classRooms as $id => $name)
                                            <option value="{{ $id }}" {{ (string) old('class_room_id', $student->class_room_id) === (string) $id ? 'selected' : '' }}>{{ $name }}</option>
                                        @endforeach
                                    </select>
                                    @error('class_room_id')
                                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                <div>
                                    ...
                                    <textarea name="allergies" id="allergies" rows="2" placeholder="Peanuts, bee stings, etc."
                                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">{{ old('allergies', $student->allergies) }}</textarea>
                                </div>
                                <div>
                                    ...
                                    <textarea name="dietary_restrictions" id="dietary_restrictions" rows="2" placeholder="Vegetarian, no pork, etc."
                                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">{{ old('dietary_restrictions', $student->dietary_restrictions) }}</textarea>
                                </div>
                                <div>
                                    ...
                                    <textarea name="medical_notes" id="medical_notes" rows="3" placeholder="Asthma inhaler, seizure plan, etc."
                                        class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">{{ old('medical_notes', $student->medical_notes) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        ...
                        <button type="submit"
                            ...
                            {{ $isChurch ? 'Update Child' : 'Update Student' }}
                        </button>
                    </div>
                </form>
            </div>
