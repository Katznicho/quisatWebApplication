    ...
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-5xl w-full space-y-8">
            ...
            <form class="mt-8 space-y-8" method="POST" action="{{ route('business.register') }}" enctype="multipart/form-data">
                ...
                <div id="supporting-documents-section" class="bg-white shadow-xl rounded-2xl p-8 border border-gray-100 hidden">
                    <div class="flex items-center mb-6">
                        <div class="flex-shrink-0">
                        ...
                        <div class="ml-4">
                            <h3 class="text-xl font-semibold text-gray-900">Supporting Documents</h3>
                            <p class="text-sm text-gray-500">Upload the documents required for your business category</p>
                        </div>
                    </div>

                    <div id="supporting-documents-list" class="space-y-5"></div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (document.getElementById('business_category_id')) {
                const categoryTomSelect = new TomSelect('#business_category_id', {
                    ...selectConfig,
                    placeholder: 'Search business category...',
                });

                categoryTomSelect.on('change', function (value) {
                    loadCategoryDocuments(value);
                });

                if (categoryTomSelect.getValue()) {
                    loadCategoryDocuments(categoryTomSelect.getValue());
                }
            }
        ...
        async function loadCategoryDocuments(categoryId) {
            const section = document.getElementById('supporting-documents-section');
            const list = document.getElementById('supporting-documents-list');
            ...
            try {
                const response = await fetch(`/business/register/categories/${categoryId}/documents`);
                ...
                list.innerHTML = documents.map((doc) => `
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            ${escapeHtml(doc.name)} ${doc.is_required ? '<span class="text-red-500">*</span>' : '<span class="text-gray-400">(Optional)</span>'}
                        </label>
                        ${doc.description ? `<p class="text-sm text-gray-500 mb-2">${escapeHtml(doc.description)}</p>` : ''}
